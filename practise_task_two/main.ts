...
import TaskTimer from "./modules/task_timer.js";
import { ITask } from "./modules/task_interface.js";
...
let time_input = <HTMLInputElement> document.querySelector('.add-task-props-popup_input');
...
if (add_task_input && add_task_button && time_input) {
    add_task_input.addEventListener('keypress', (event: KeyboardEvent) => {
        if (event.key === 'Enter') {
            ...
            time_input.addEventListener('keypress', (event: KeyboardEvent) => {
                if (event.key === 'Enter') {
